...
func TestNewNodeJSParser(t *testing.T) {
	...
	if parser == nil {
		t.Error("NewNodeJSParser() returned nil")
	}
}

func TestNodeJSParserLanguage(t *testing.T) {
	parser := NewNodeJSParser()
	if parser.Language() != "nodejs" {
		t.Errorf("Language() = %q, want %q", parser.Language(), "nodejs")
	}
}

func TestNodeJSParserParseBasic(t *testing.T) {
	data, err := os.ReadFile("../../testdata/nodejs/benchmark_js_basic.txt")
	if err != nil {
	...
	if suite.Language != "nodejs" {
		t.Errorf("suite.Language = %q, want %q", suite.Language, "nodejs")
	}
	...
	if suite.Results[0].Name != "Array#forEach" {
		t.Errorf("first result Name = %q, want %q", suite.Results[0].Name, "Array#forEach")
	}
...
func TestNodeJSParserParseEdgeCases(t *testing.T) {
	data, err := os.ReadFile("../../testdata/nodejs/benchmark_js_edge_cases.txt")
	if err != nil {
		t.Skipf("Skipping test - testdata file not found: %v", err)
		return
	}
...
func TestNodeJSParserParseWithFastest(t *testing.T) {
	data, err := os.ReadFile("../../testdata/nodejs/benchmark_js_with_errors.txt")
	if err != nil {
		t.Skipf("Skipping test - testdata file not found: %v", err)
		return
	}
...
func TestNodeJSParserTableDriven(t *testing.T) {
	tests := []struct {
		name          string
		input         string
		expectedCount int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.expectedErr {
				...
				if suite.Language != "nodejs" {
					t.Errorf("Language = %q, want %q", suite.Language, "nodejs")
				}
...
func TestNodeJSParserTimestampIsSet(t *testing.T) {
	parser := NewNodeJSParser()
...
func TestNodeJSParserMetadataPresent(t *testing.T) {
	parser := NewNodeJSParser()
	suite, err := parser.Parse([]byte("test x 1,000 ops/sec ±1.0% (10 runs sampled)"))
	if err != nil {
		t.Fatalf("Parse() returned error: %v", err)
...
func TestNodeJSParserErrorHandling(t *testing.T) {
	tests := []struct {
		name        string
		input       string
		expectError bool
...
func TestNodeJSParserResultValues(t *testing.T) {
	...
	if result.Language != "nodejs" {
		t.Errorf("Language = %q, want %q", result.Language, "nodejs")
	}
